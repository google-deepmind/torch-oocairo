...
used as the live buffer. Use C<image_surface_create_for_data> to avoid this
behaviour.
...
=item image_surface_create_for_data (data, format, width, height, stride)

Same as C<image_surface_create_from_data> but using the memory pointed by data
as the buffer instead of making a copy of it, replicating the original
behaviour of the native Cairo function C<cairo_image_surface_create_for_data>.
...
(I<x1>,E<nbsp>I<y1>) to the point (I<x2>,E<nbsp>I<y2>).

=item pattern_create_radial (x1, y1, r1, x2, y2, r2)

Return a new pattern object representing a radial gradient, where offsets
move from the circle defined by the three numbers (I<x1>,E<nbsp>I<y1>,E<nbsp>I<r1>),
where I<r1> is the radius, to the circle (I<x2>,E<nbsp>I<y2>,E<nbsp>I<r2>).
