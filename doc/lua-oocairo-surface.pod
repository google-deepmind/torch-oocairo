...
There are actually three values returned from this method, which between
them are enough to decode the data:

=over

=item 1

The data itself as a Lua string, which is potentially quite large.

=item 2

The I<stride>, which is the number of bytes of the string used for each
...
A string indicating which order the pixels are stored in.  This will be
C<argb> on big-endian systems and C<bgra> on little-endian ones.  This
is the order in which the bytes for each pixel are stored on C<rgb24> and
C<argb32> image surfaces.  For C<a1> it indicates the order the pixels are
encoded as bits.  It can be ignored when the image format is C<a8>.
...
in the order given by the last value returned from the method.  On C<rgb24>
